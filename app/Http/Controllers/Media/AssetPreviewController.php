...
class AssetPreviewController extends Controller
{
    ...
    {
        $owner = Workspace::query()->findOrFail($workspace);
        $asset = FindWorkspaceAsset::execute($owner, $media);

        abort_if($asset === null, Response::HTTP_NOT_FOUND);

        $disk = Storage::disk((string) config('filesystems.default', 'local'));

        abort_unless($disk->exists($asset->path), Response::HTTP_NOT_FOUND);

        if ($disk instanceof LocalFilesystemAdapter && $path = $this->localPath($disk, $asset->path)) {
            return response()->file($path, $this->headers($asset));
        }
        ...
        }, Response::HTTP_OK, $this->headers($asset));
    }

    /**
     * Returns an absolute local filesystem path only when the resolved asset
     * path remains inside the configured disk root.
     */
