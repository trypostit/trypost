        ...
        return [
            ...
            'platforms' => $schema->array()
                ->items($schema->object(fn ($p) => [
                    ...
                    'meta' => $p->object()->description('Per-platform metadata override. Instagram/Facebook: aspect_ratio. TikTok: privacy_level (required to publish) + flags. Pinterest: board_id (required to publish — call ListPinterestBoardsTool first), title (≤100), description (≤800; seeded from content when omitted), link (destination URL). Discord: channel_id (required to publish — call ListDiscordChannelsTool first), mentions, embeds. Merged with existing meta.'),
                ]))
