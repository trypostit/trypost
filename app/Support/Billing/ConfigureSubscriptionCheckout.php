...
final class ConfigureSubscriptionCheckout
{
     ...
     * 2. Else CASHIER_TRIAL_DAYS > 0 → trialDays (clamped to ≥ 2).
     * 3. Else plain checkout (immediate full price).
     *
        ...
        if (self::shouldApplyFirstMonthCoupon($account)) {
            if ((bool) config('cashier.allow_promotion_codes', true)) {
                throw new RuntimeException(
                    'STRIPE_FIRST_MONTH_COUPON_ID and CASHIER_ALLOW_PROMOTION_CODES cannot both be enabled: '
