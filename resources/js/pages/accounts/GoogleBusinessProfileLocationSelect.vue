        ...
        <div class="flex flex-col gap-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold tracking-tight">
                        {{ $t('accounts.google_business_profile.title') }}
                    </h1>
                    <p class="text-sm text-muted-foreground">
                        {{ $t('accounts.google_business_profile.description') }}
                    </p>
                </div>
            </div>
            ...
            <div v-if="locations.length === 0" class="py-12 text-center">
                <h3 class="text-lg font-semibold">
                    {{ $t('accounts.google_business_profile.no_locations') }}
                </h3>
                <p class="text-sm text-muted-foreground">
                    {{
                        $t(
                            'accounts.google_business_profile.no_locations_description',
                        )
                    }}
                </p>
            </div>
            ...
            <div v-else class="grid gap-3">
                <button
                    ...
                    :class="{
                        'border-blue-500 ring-2 ring-blue-500/20':
                            form.location_ids.includes(location.id),
                    }"
                ...
                >
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p
                            v-if="location.store_code"
                            class="text-xs text-muted-foreground"
                        >
                            {{
                                $t(
                                    'accounts.google_business_profile.store_code',
            ...
            >
                {{
                    form.processing
                        ? $t('accounts.google_business_profile.saving')
                        : $t('accounts.google_business_profile.save')
                }}
            </Button>
        </div>
    </PopupLayout>
