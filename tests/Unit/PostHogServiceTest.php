...
test('captureOnce dispatches only once for the same dedupe key', function () {
    Queue::fake();
    config(['services.posthog.enabled' => true, 'services.posthog.api_key' => 'phc_test_key']);

    $service = new PostHogService;
    $service->captureOnce('posthog:test:once', 'user-123', 'once.event');
    $service->captureOnce('posthog:test:once', 'user-123', 'once.event');

    Queue::assertPushed(SendEvent::class, 1);
});

test('captureOnce does not claim the cache when PostHog is disabled', function () {
    Queue::fake();
    config(['services.posthog.enabled' => false, 'services.posthog.api_key' => 'phc_test_key']);

    $service = new PostHogService;
    $service->captureOnce('posthog:test:disabled', 'user-123', 'once.event');
...
test('captureOnce is a no-op when when is false', function () {
    Queue::fake();
    config(['services.posthog.enabled' => true, 'services.posthog.api_key' => 'phc_test_key']);

    $service = new PostHogService;
    $service->captureOnce('posthog:test:when', 'user-123', 'once.event', when: false);

    Queue::assertNothingPushed();

    $service->captureOnce('posthog:test:when', 'user-123', 'once.event', when: true);
